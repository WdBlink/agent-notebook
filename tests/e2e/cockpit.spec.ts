...
test("copies a project-aware resume command", async ({ page }) => {
  await page.goto(harnessUrl);
  const session = page.locator(".agent-notebook-session").filter({ hasText: "实现每日看板连续性原型" });
  const button = session.locator(".agent-notebook-resume");

  await button.click();
  await expect(button).toHaveText("已复制");
  const command = await page.evaluate(
    () => (window as Window & { agentNotebookCopiedResumeCommand?: string }).agentNotebookCopiedResumeCommand
  );
  expect(command).toBe('cd "$HOME/Documents/agent-notebook" && codex resume seed-codex-session');
});
...
test("decomposes intent into ordinary completable tasks", async ({ page }) => {
  ...
  await expect(page.locator(".agent-notebook-task").filter({ hasText: "查清项目核心概念" })).toBeVisible();
  await expect(page.locator(".agent-notebook-task").filter({ hasText: "尝试跑 demo" })).toBeVisible();
  const task = page.locator(".agent-notebook-task").filter({ hasText: "尝试跑 demo" });
...
test("long todo lists use internal scroll containers", async ({ page }) => {
  ...
  const taskList = page.locator(".agent-notebook-task-list");
...
test("task updates preserve list scroll, focused draft, and selection", async ({ page }) => {
  ...
  const state = await page.evaluate(() => {
    const list = document.querySelector(".agent-notebook-task-list");
...
test("export panel reports path", async ({ page }) => {
  ...
  await expect(page.getByText("Agent Notebook/2026-07-04.md")).toBeVisible();
});
