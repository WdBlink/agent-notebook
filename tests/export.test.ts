...
import { buildDailyMarkdown, dailyNotePath, isAgentNotebookMarkdown } from "../src/export";
...
test("daily note path uses the active plan target date", () => {
  ...
  assert.equal(path, "Agent Notebook/2026-07-04.md");
});
...
test("daily markdown includes prior agent sessions before intent", () => {
  ...
  assert.ok(markdown.includes('resume: cd "$HOME/Documents/agent-notebook" && codex resume seed-codex-session'));
});
...
test("empty export still writes structured note", () => {
  ...
  assert.ok(markdown.includes("# Agent Notebook 2026-07-03"));
...
test("plugin-owned marker is required before overwriting an existing export", () => {
  assert.equal(isAgentNotebookMarkdown(buildDailyMarkdown(seededData())), true);
  assert.equal(isAgentNotebookMarkdown("---\nsource: handwritten\n---\n# My note\n"), false);
  assert.equal(isAgentNotebookMarkdown("# No frontmatter\n"), false);
});
